...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<ErrorMessage> updateTaskError(TaskUpdateException exception ) {
        ErrorMessage error = new ErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), exception.getMessage(),"Couldn't update Task on the database");
        return new ResponseEntity<ErrorMessage>(error,new HttpHeaders(),HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    public final ResponseEntity<ErrorMessage> deleteTaskError(TaskDeleteException exception ) {
        ErrorMessage error = new ErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), exception.getMessage(),"Couldn't delete Task from the database");
        return new ResponseEntity<ErrorMessage>(error,new HttpHeaders(),HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
